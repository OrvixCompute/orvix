...
### Added
- **Non-custodial user staking (Anchor program):** new `staking-program/` — an Anchor v2 Solana program where users lock ORVX in a program-owned vault for a user-chosen lock period (3/7/14 days). No operator key can move staked tokens; unstaking is only allowed after the lock expires. Orchestrator routes `POST /v1/staking/user/stake|unstake` build unsigned transactions for the user's wallet to sign, `POST /v1/staking/user/submit` broadcasts the signed transaction, and `GET /v1/staking/user/status` reads the on-chain `StakeAccount` PDA and derives the tier. Opt-in via `USER_STAKING_PROGRAM_ID` (empty disables the routes). Provider staking is untouched — this is a separate, non-custodial path for users. `staking-program/build.sh` builds against the Solana platform-tools `sbpfv1` target so `anchor deploy` finds a loader-compatible artifact
- **Opt-in OpenCovenant node attestation:** at registration, the orchestrator can run a read-only reputation check (`covenant_reputation`) against the OpenCovenant MCP endpoint and store the verdict on the `nodes` row (`covenant_attestation` JSONB, migration `021`). Strictly opt-in — `COVENANT_ENABLE_ATTESTATION=false` (default) or an empty `COVENANT_PROVIDER_WALLET_ADDRESS` leaves registration exactly as before. The check is fail-soft: a network error or timeout records "no attestation" but never blocks a node. New config `COVENANT_MCP_URL`, `COVENANT_MCP_TIMEOUT_S`, `COVENANT_MIN_REPUTATION`, `COVENANT_ENABLE_ATTESTATION`, `COVENANT_PROVIDER_WALLET_ADDRESS`
