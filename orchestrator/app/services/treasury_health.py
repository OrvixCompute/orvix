    ...
    try:
        for role in ("payout", "hot"):
            ...
            if sol_balance < Decimal(str(min_sol)):
                # Out of SOL is worse than out of USDC: without it the payout
                # cannot even be broadcast, so every withdrawal fails and
                # refunds rather than queueing until funds arrive.
                alerts.append(
                    _alert(
                        CRITICAL,
                        role,
