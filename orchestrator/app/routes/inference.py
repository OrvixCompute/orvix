    ...
    inserted = db.table("jobs").insert(
        {
            ...
            "node_id": node_id,
            # Stored on the job itself, not inferred through the node: node rows
            # get deleted and (since migration 015) that nulls node_id, which
            # would otherwise take the earnings attribution with it.
